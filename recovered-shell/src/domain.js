...
export function createProject(name = "Pentagory", starter = true) {
  ...
  const project = {
    ...
    execution: [],
    members: starter ? [
      { id: uid(), name: "Deimox", role: "Creative Director", specialties: ["design", "code"], permissions: [...STAGES] },
      { id: uid(), name: "Iki", role: "Producer", specialties: ["production"], permissions: [...STAGES] }
    ] : [],
    history: []
  };
...
export function saveMember(project, { id, name, role, specialties }) {
  ...
  Object.assign(member, { name, role: String(role || "Colaborador").trim().slice(0, 100), specialties: [...new Set(specialties.filter(value => CATEGORIES.some(([key]) => key === value)))], permissions: [...STAGES] });
