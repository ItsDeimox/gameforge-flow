...
const TEAM_MEMBER_PERMISSIONS = [["vision", "Visão"], ["production", "Produção"], ["execution", "Execução"], ["comments", "Comentários"], ["files", "Arquivos"], ["team", "Gerenciar equipe"], ["settings", "Configurações"]];
const TEAM_MEMBER_DEFAULTS = ["vision", "production", "execution", "comments", "files"];
...
const collaboration = new Collaboration({
  ...
  getPresence: () => ({
    stage: state.currentScreen === "work" ? state.currentStage : state.currentScreen,
    editing: $("#modalBackdrop")?.classList.contains("hidden") ? "" : ($("#modal h3")?.textContent || $("#modal .eyebrow")?.textContent || "")
  }),
...
function canCollaborate(permission) {
  return typeof collaboration.can === "function" ? collaboration.can(permission) : (!collaboration.active || collaboration.admin);
}
...
function renderVision() {
  ...
  $("#docCollaborationBtn").innerHTML = `<span aria-hidden="true">◎</span> COMENTÁRIOS E ARQUIVOS <b>${(doc.comments || []).length + (doc.attachments || []).length}</b>`;
  $("#docCollaborationBtn").onclick = () => openContentCollaboration("doc", doc);
  const canEditVision = canCollaborate("vision");
  $("#docTitle").disabled = !canEditVision; $("#docStatusSelect").disabled = !canEditVision; $("#visionEditor").readOnly = !canEditVision; $("#liveEditor").contentEditable = String(canEditVision); $("#newDocBtn").disabled = !canEditVision;
  $$(".markdown-toolbar button").forEach(button => { button.disabled = !canEditVision; });
...
function sendSelectionToProduction() {
  if (!requirePermission("vision", "Seu acesso à Visão é somente leitura.")) return;
...
function renderProduction() {
  const project = currentProject();
  const canEditProduction = canCollaborate("production");
  $("#newDemandBtn").disabled = !canEditProduction;
  const lanes = [
  ...
  $$(".production-demand").forEach(card => {
    card.draggable = canEditProduction;
  ...
  $$('[data-prod-drop]').forEach(stack => {
    stack.ondragover = event => { if (!canEditProduction) return; event.preventDefault(); stack.classList.add("drop-active"); };
    stack.ondragleave = () => stack.classList.remove("drop-active");
    stack.ondrop = event => {
      if (!canEditProduction) return; event.preventDefault(); stack.classList.remove("drop-active");
...
function editProductionCard(id = null) {
    ...
    <div class="demand-editor-shell">
      ...
      <div class="demand-editor-body">
        ...
        <aside class="demand-properties" aria-label="Dados da demanda">
          ...
          ${id ? contentCollaborationMarkup("production", item) : ""}
        </aside>
      </div>
  ...
  if (id) bindContentCollaboration($("#modalContent").querySelector?.(".content-collaboration"), item);
  productionEditor = bindProductionEditor();
  makeEditorReadOnly("production");
  $("#saveProdBtn").onclick = () => {
...
function renderExecution() {
  const project = currentProject();
  const canEditExecution = canCollaborate("execution");
  $("#newTaskBtn").disabled = !canEditExecution;
  ...
  $$(".execution-mission").forEach(card => {
    card.draggable = canEditExecution;
    card.ondragstart = event => { card.classList.add("dragging"); event.dataTransfer.setData("text/plain", card.dataset.id); sound.play("ui_drag_start"); };
    card.ondragend = () => card.classList.remove("dragging");
  ...
  $$('[data-exec-drop]').forEach(stack => {
    stack.ondragover = event => { if (!canEditExecution) return; event.preventDefault(); stack.classList.add("drop-active"); };
    stack.ondragleave = () => stack.classList.remove("drop-active");
    stack.ondrop = event => {
      if (!canEditExecution) return; event.preventDefault(); stack.classList.remove("drop-active");
...
function editTask(id = null) {
  ...
  openModal(`<span class="eyebrow">EXECUÇÃO · TAREFA</span><h3>${id ? "Detalhes da tarefa" : "Nova tarefa"}</h3><p>Defina o responsável, o prazo e os próximos passos.</p>
    ...
    ${id ? contentCollaborationMarkup("execution", item) : ""}
...
function contentTarget(project, targetType, targetId) {
  const key = ({ doc: "docs", production: "production", execution: "execution" })[targetType];
  return project?.[key]?.find(item => item.id === targetId);
}
function fileSize(bytes = 0) {
  return bytes < 1024 ? `${bytes} B` : bytes < 1048576 ? `${Math.round(bytes / 1024)} KB` : `${(bytes / 1048576).toFixed(1)} MB`;
}
function contentCollaborationMarkup(targetType, item) {
  const comments = item.comments || [], attachments = item.attachments || [];
  return `<section class="content-collaboration" data-collaboration-target="${esc(targetType)}" data-collaboration-id="${esc(item.id)}">
    <header><div><span>COMUNICAÇÃO</span><b>${comments.length} comentário(s) · ${attachments.length} arquivo(s)</b></div><i aria-hidden="true"></i></header>
    <div class="content-comments">${comments.length ? comments.map(comment => `<article><div><b>${esc(comment.author || "Equipe")}</b><time>${comment.createdAt ? new Date(comment.createdAt).toLocaleString("pt-BR") : ""}</time></div><p>${esc(comment.text)}</p></article>`).join("") : "<p class=\"content-empty\">Nenhuma mensagem ainda.</p>"}</div>
    <div class="comment-composer"><textarea maxlength="4000" placeholder="Escreva para a equipe…" aria-label="Novo comentário"></textarea><button type="button" data-add-comment ${canCollaborate("comments") && canCollaborate(targetType === "doc" ? "vision" : targetType) ? "" : "disabled"}>ENVIAR</button></div>
    <div class="content-files">${attachments.map(file => `<button type="button" data-download-file="${esc(file.id)}" title="Baixar ${esc(file.name)}"><span>↧</span><b>${esc(file.name)}</b><small>${fileSize(file.size)}</small></button>`).join("") || "<p class=\"content-empty\">Sem arquivos anexados.</p>"}</div>
    <label class="content-upload ${canCollaborate("files") && canCollaborate(targetType === "doc" ? "vision" : targetType) ? "" : "disabled"}"><input type="file" data-add-file ${canCollaborate("files") && canCollaborate(targetType === "doc" ? "vision" : targetType) ? "" : "disabled"}><span>＋ ANEXAR ARQUIVO</span><small>${collaboration.active ? "até 6 MB" : "até 1,5 MB no modo local"}</small></label>
    <p class="content-feedback" role="status"></p>
  </section>`;
}
function refreshContentCollaboration(section, targetType, item) {
  const holder = document.createElement("div"); holder.innerHTML = contentCollaborationMarkup(targetType, item);
  const fresh = holder.firstElementChild; section.replaceWith(fresh); bindContentCollaboration(fresh, item);
}
function readFileData(file) {
  return new Promise((resolve, reject) => { const reader = new FileReader(); reader.onerror = () => reject(new Error("Não foi possível ler o arquivo.")); reader.onload = () => resolve(reader.result); reader.readAsDataURL(file); });
}
function bindContentCollaboration(section, item) {
  if (!section) return;
  const targetType = section.dataset.collaborationTarget;
  section.querySelector("[data-add-comment]").onclick = async () => {
    const field = section.querySelector("textarea"), feedback = section.querySelector(".content-feedback"), text = field.value.trim();
    if (!text) return field.focus();
    try {
      feedback.textContent = "ENVIANDO…";
      if (collaboration.active) {
        const result = await collaboration.addComment(targetType, item.id, text), remote = contentTarget(result.snapshot.project, targetType, item.id);
        Object.assign(item, remote); currentProject().history = result.snapshot.project.history;
      } else {
        item.comments ||= []; item.comments.push({ id: uid(), text, author: "Você", authorId: null, createdAt: now() }); addHistory(currentProject(), "COMMENT_ADDED", `Comentário em “${item.title || item.text}”`); persist();
      }
      refreshContentCollaboration(section, targetType, item); sound.play("ui_notification");
    } catch (error) { feedback.textContent = error.message; }
  };
  section.querySelector("[data-add-file]").onchange = async event => {
    const file = event.target.files?.[0], feedback = section.querySelector(".content-feedback"); if (!file) return;
    try {
      feedback.textContent = "ENVIANDO ARQUIVO…";
      if (collaboration.active) {
        const result = await collaboration.uploadFile(targetType, item.id, file), remote = contentTarget(result.snapshot.project, targetType, item.id);
        Object.assign(item, remote);
      } else {
        if (file.size > 1_500_000) throw new Error("No modo local, use arquivos de até 1,5 MB.");
        item.attachments ||= []; item.attachments.push({ id: uid(), name: file.name, mime: file.type || "application/octet-stream", size: file.size, author: "Você", createdAt: now(), data: await readFileData(file), remote: false }); persist();
      }
      refreshContentCollaboration(section, targetType, item); sound.play("ui_confirm");
    } catch (error) { feedback.textContent = error.message; event.target.value = ""; }
  };
  section.querySelectorAll("[data-download-file]").forEach(button => button.onclick = async () => {
    const file = (item.attachments || []).find(candidate => candidate.id === button.dataset.downloadFile); if (!file) return;
    try {
      if (file.remote && collaboration.active) await collaboration.downloadFile(file.id, file.name);
      else if (file.data) { const link = document.createElement("a"); link.href = file.data; link.download = file.name; link.click(); }
      else throw new Error("Este anexo pertence a uma sessão compartilhada. Reconecte para baixar.");
    } catch (error) { section.querySelector(".content-feedback").textContent = error.message; }
  });
}
function openContentCollaboration(targetType, item) {
  openModal(`<span class="eyebrow">EQUIPE · COMUNICAÇÃO</span><h3>${esc(item.title || item.text || "Conteúdo")}</h3><p>Decisões, contexto e arquivos ficam ligados a este item.</p>${contentCollaborationMarkup(targetType, item)}<div class="modal-actions"><button class="hud-button" data-close>FECHAR</button></div>`, "modal-wide collaboration-modal");
  bindContentCollaboration($("#modalContent").querySelector?.(".content-collaboration"), item);
}
function makeEditorReadOnly(permission) {
  if (canCollaborate(permission)) return;
  const root = $("#modalContent"), communication = root.querySelector?.(".content-collaboration");
  root.querySelectorAll?.("input, select, textarea, button").forEach(control => {
    if (!control.closest?.(".content-collaboration") && !control.matches?.("[data-close], #prodFullscreenBtn")) control.disabled = true;
  });
  root.querySelectorAll?.("[contenteditable]").forEach(editor => { editor.contentEditable = "false"; });
  if (communication) communication.dataset.editorReadOnly = "true";
}
function requirePermission(permission, message) {
  if (canCollaborate(permission)) return true;
  toast(message || "Seu acesso não permite esta ação."); return false;
}
function requireTeamAdmin() {
  return requirePermission("team", "Seu acesso não permite gerenciar a equipe deste Save.");
}
function openTeam() {
  const project = currentProject();
  openModal(`<span class="eyebrow">SAVE · EQUIPE</span><h3>Equipe de ${esc(project.name)}</h3><p>Cargos são livres. Especialidades organizam a escolha de responsáveis nas próximas etapas.</p>
    ...
    <div class="modal-actions"><button class="hud-button" data-close>FECHAR</button>${canCollaborate("team") ? '<button id="addMemberBtn" class="accent-button">ADICIONAR PESSOA</button>' : ""}</div>`, "modal-wide team-modal");
  if (canCollaborate("team")) {
...
function editMember(id) {
  ...
  const member = currentProject().members.find(member => member.id === id) || { name: "", role: "", specialties: [], permissions: TEAM_MEMBER_DEFAULTS };
  openModal(`<span class="eyebrow">EQUIPE · MEMBRO</span><h3>${id ? "Editar pessoa" : "Adicionar pessoa"}</h3><p>Uma pessoa pode atuar em várias especialidades.</p>
    ...
    <fieldset class="specialty-picker permission-picker"><legend>ACESSO NESTE SAVE</legend>${TEAM_MEMBER_PERMISSIONS.map(([key, label]) => `<label class="team-check" for="permission-${key}"><input id="permission-${key}" type="checkbox" value="${key}" name="memberPermission" ${(member.permissions || TEAM_MEMBER_DEFAULTS).includes(key) ? "checked" : ""}><span>${esc(label)}</span></label>`).join("")}</fieldset>
  ...
  $("#saveMemberBtn").onclick = () => {
    try {
      saveMember(currentProject(), { id, name: $("#memberName").value, role: $("#memberRole").value, specialties: $$('input[name="specialty"]:checked').map(input => input.value), permissions: $$('input[name="memberPermission"]:checked').map(input => input.value) });
      persist(); openTeam();
...
function openSettings() {
  ...
  $("#saveSettingsBtn").onclick = () => {
    if (!requirePermission("settings", "Seu acesso não permite alterar as configurações deste Save.")) return;
...
function renderPresence(rows = collaboration.presence || []) {
  const indicator = $("#presenceIndicator");
  if (!indicator) return;
  const people = [...new Map(rows.map(row => [row.memberId || `host:${row.clientId}`, row])).values()];
  indicator.classList.toggle("hidden", !collaboration.active);
  indicator.querySelector("span").textContent = String(people.length);
  indicator.title = people.length ? people.map(person => `${person.name} · ${person.stage}${person.editing ? ` · ${person.editing}` : ""}`).join("\n") : "Ninguém online agora";
  const detail = $("#connectionPresence");
  if (detail) detail.innerHTML = people.length ? `<span>ONLINE AGORA</span>${people.map(person => `<article><i></i><div><b>${esc(person.name)}</b><small>${esc(person.role)} · ${esc(person.stage)}${person.editing ? ` · ${esc(person.editing)}` : ""}</small></div></article>`).join("")}` : "<p>Nenhuma presença ativa detectada.</p>";
}
...
function openConnection() {
  ...
  openModal(`<span class="eyebrow">EQUIPE · REDE</span><h3>${shared ? "Save compartilhado" : "Conexão"}</h3><p>O host guarda o Save. A equipe abre o endereço dele pela rede, Radmin VPN ou um túnel HTTPS.</p>
    ${shared ? `<p id="connectionDetailStatus" class="connection-state"></p><p>${esc(collaboration.principal.name)} · ${collaboration.admin ? "HOST / ADMINISTRADOR" : esc(collaboration.principal.role || "COLABORADOR")} · revisão ${collaboration.revision} · sala <code>${esc(collaboration.session.roomId)}</code></p>
      <div id="connectionPresence" class="connection-presence"></div>
      ${canCollaborate("team") ? `<div class="form-row full"><label class="form-field"><span>ENDEREÇO PARA A EQUIPE · IP DA VPN OU URL HTTPS</span><input id="inviteBase" value="${esc(sessionStorage.getItem("gf-public-origin") || location.origin)}" placeholder="http://26.x.x.x:8765"></label></div>
  ...
  if (shared) {
    ...
    const getHost = async () => {
      ...
      $("#hostInfo").innerHTML = `<div class="network-discovery"><span>ENDEREÇOS ENCONTRADOS</span>${(info.networks || info.addresses.map(address => ({ address, kind: "Rede local" }))).map(network => `<button type="button" data-use-address="${esc(network.address)}"><b>${esc(network.kind)}</b><code>http://${esc(network.address)}:8765</code></button>`).join("") || "<p>Informe manualmente o IP da VPN no convite.</p>"}<small>${(info.tunnels || []).map(tunnel => `${esc(tunnel.name)}: ${tunnel.available ? "detectado" : "não instalado"}`).join(" · ")}</small></div>${info.rooms.map(room => `<button class="hud-button" data-host-room="${esc(room.roomId)}">RETOMAR ${esc(room.name)}</button>`).join("")}`;
      $$('[data-use-address]').forEach(button => button.onclick = () => { const address = `http://${button.dataset.useAddress}:8765`; sessionStorage.setItem("gf-public-origin", address); const field = $("#inviteBase"); if (field) field.value = address; button.classList.add("selected"); toast("Endereço escolhido para o próximo convite."); });
...
function bindEvents() {
  ...
  $("#soundToggle").onclick = () => { if (!requirePermission("settings", "Seu acesso não permite alterar o som deste Save.")) return; currentProject().settings.sound = !currentProject().settings.sound; updateSoundLabel(); persist(); if (currentProject().settings.sound) sound.play("ui_click"); };
  ...
  $("#presenceIndicator").onclick = openConnection;
...
function boot() {
  ...
  if ("serviceWorker" in navigator) navigator.serviceWorker.register("./service-worker.js?v=0.9.0", { updateViaCache: "none" }).then(registration => registration.update()).catch(() => {});
